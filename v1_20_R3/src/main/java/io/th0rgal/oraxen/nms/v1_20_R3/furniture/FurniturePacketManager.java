...
public class FurniturePacketManager implements IFurniturePacketManager {
    ...
    private Packet<?> handlePacket(Packet<?> packet, Connection connection) {
        return switch (packet) {
            case ClientboundBundlePacket bundlePacket -> {
                List<Packet<ClientGamePacketListener>> newBundle = new ArrayList<>();
                bundlePacket.subPackets().forEach(bundle -> newBundle.add((Packet<ClientGamePacketListener>) handlePacket(bundle, connection)));
    ...
    public void removeInteractionHitboxPacket(@NotNull ItemDisplay baseEntity, @NotNull FurnitureMechanic mechanic, @NotNull Player player) {
        for (FurnitureSubEntity s : interactionHitboxIdMap) {
            if (!s.baseUUID().equals(baseEntity.getUniqueId())) continue;
            ((CraftPlayer) player).getHandle().connection.send(new ClientboundRemoveEntitiesPacket(s.entityIds()));
            break;
        }
    }
