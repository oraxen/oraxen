...
### Private Resources (Supabase Storage)

Default resource pack assets (`core/src/main/resources.zip`) are stored in a private Supabase Storage bucket and are gitignored. This allows new clones to bootstrap without committing binary assets.

- **Bucket**: `oraxen_resources` on `dmtwdyasmifhjsxmqslv.supabase.co`
- **File**: `resources.zip` (contains the full `core/src/main/resources/` directory)
- **Auth**: Requires `SUPABASE_SERVICE_ROLE_KEY` environment variable

```bash
# Download resources.zip for a fresh clone
curl -o core/src/main/resources.zip \
  "https://dmtwdyasmifhjsxmqslv.supabase.co/storage/v1/object/oraxen_resources/resources.zip" \
  -H "Authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY"

# Upload updated resources.zip after changing assets
cd core/src/main/resources && zip -r ../resources.zip . && cd -
curl -X PUT \
  "https://dmtwdyasmifhjsxmqslv.supabase.co/storage/v1/object/oraxen_resources/resources.zip" \
  -H "Authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY" \
  -H "Content-Type: application/zip" \
  --data-binary @core/src/main/resources.zip
```
