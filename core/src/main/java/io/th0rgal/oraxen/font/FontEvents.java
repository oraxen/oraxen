...
public class FontEvents implements Listener {
    ...
    public enum ChatHandler {
        ...
        public static ChatHandler get() {
            return Settings.CHAT_HANDLER.toEnumOrGet(ChatHandler.class, (handler) -> {
                ChatHandler chatHandler = VersionUtil.isPaperServer() ? MODERN : LEGACY;
                Logs.logError("Invalid chat-handler " + handler + " defined in settings.yml, defaulting to " + chatHandler, true);
