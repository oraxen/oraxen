...
public class DenyPlacingOn {
    ...
    public List<Tag<Material>> getDeniedTags() {
    ...
    public List<String> getDeniedOraxenBlockIds() {
    ...
    public boolean checkDeniedMechanic(Block block) {
        String oraxenId = checkIfOraxenItem(block);
        if (oraxenId == null) {
            if (getDeniedBlocks().contains(block.getType())) {
                return true;
            }
            for (Tag<Material> tag : getDeniedTags()) {
                if (tag.isTagged(block.getType())) {
                    return true;
                }
            }
        }
        return (oraxenId != null && getDeniedOraxenBlockIds().contains(oraxenId));
    }
