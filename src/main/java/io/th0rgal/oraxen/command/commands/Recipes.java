...
public class Recipes {
    public static CommandInfo build() {
        return new CommandInfo("recipe", info -> {
            ...
            builder.requires(
                Conditions.mixed(
                    Conditions.reqPerm(OraxenPermission.COMMAND_RECIPE), 
                    Conditions.player(Message.NOT_PLAYER))
                ).then(
                    Literal.of("builder").then(
                        Argument.of("type", RecipeTypeType.TYPE).optionally(
                            Argument.of("furnace", SpecificWordType.of("cookingtime", "experience"))
                        ).executes((sender, context) -> {
                            // Builder
                        })
                    )
                ).then(
                    Literal.of("save").executes((sender, context) -> {
                        // Save
                    })
                ).then(
